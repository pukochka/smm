<template>
  <div class="relative-position">
    <div v-if="currentItems.length === 0 && searching && search !== ''">
      <div
        class="absolute-full bg-item q-pa-md column flex-center text-center rounded q-gutter-y-sm bordered"
      >
    ...
    <div class="rounded overflow-hidden">
      ...
      >
        <q-tab-panel name="0" class="q-pa-none">
          <q-list
            :bordered="!(currentItems.length === 0 && searching)"
            separator
...
const height = computed(() =>
  items.value.length <= props.visibleItems && !props.stableHeight
...
const slide = (side: 'prev' | 'next') => {
  ...
  panels.value = panels.value === '0' ? '1' : '0';
};
